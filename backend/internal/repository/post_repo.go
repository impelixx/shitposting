...
func (r *PostRepo) Create(ctx context.Context, in CreatePostInput) (*Post, error) {
	...
	err := r.pool.QueryRow(ctx, `
		...
		RETURNING id, title, slug, body, excerpt, tags, cover_image, published, created_at, updated_at`,
		in.Title, in.Slug, in.Body, in.Excerpt, in.Tags, in.CoverImage, in.Published,
	).Scan(&p.ID, &p.Title, &p.Slug, &p.Body, &p.Excerpt, &p.Tags, &p.CoverImage, &p.Published, &p.CreatedAt, &p.UpdatedAt)
	return &p, err
}
...
func (r *PostRepo) List(ctx context.Context, tag string, limit, offset int) ([]*Post, error) {
	...
	var args []any
	if tag != "" {
		query = `SELECT id, title, slug, body, excerpt, tags, cover_image, published, created_at, updated_at
